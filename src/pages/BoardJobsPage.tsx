...
import { getBoardDisplayName, getJobsForBoard } from '../utils/categoryUtils';
import { 
  Search, ArrowLeft, RotateCcw, LayoutGrid, TableProperties,
  Sparkles, AlertCircle, ChevronDown, CheckCircle2, Building2
} from 'lucide-react';
...
export default function BoardJobsPage() {
  ...
  const boardName = getBoardDisplayName(slug, JOBS_DATA);
  ...
  const boardActiveJobs = useMemo(() => {
    const activeJobs = JOBS_DATA.filter(job => !isJobExpired(job.l));
    return getJobsForBoard(activeJobs, slug);
  }, [slug]);

  if (!boardName && boardActiveJobs.length === 0) {
    return <Navigate to="/" replace />;
  }

  const filteredJobs = useMemo(() => {
    let result = boardActiveJobs.filter(job => {
      const matchesSearch = 
    ...
    return result;
  }, [boardActiveJobs, searchTerm, sortBy]);

  useEffect(() => {
    setVisibleCount(10);
  }, [searchTerm, sortBy, slug]);

  const title = `${boardName} Recruitment 2026`;
  const seoTitle = `${title} | Free Job Alerts | NewVacancyAlert`;
  const description = `Verified active public sector vacancies, apprenticeships, and Sarkari jobs for ${boardName}.`;
  ...
  return (
    <main className="flex-1 flex flex-col w-full bg-slate-50 min-h-screen">
      <Helmet>
        ...
        <meta name="keywords" content={`${boardName} Recruitment 2026, ${boardName} Jobs, ${boardName} Vacancy`} />
        <link rel="canonical" href={`https://newvacancyalert.in/board/${slug}`} />
      </Helmet>

      {/* Header Banner */}
      <section className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white py-8 px-4 sm:px-6 relative overflow-hidden border-b-4 border-blue-500">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className="text-xs font-black px-2.5 py-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-400/30">
                  {boardActiveJobs.length} Active Vacancies
                </span>
              </div>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
          ...
          <div 
            ...
            className="relative flex items-center bg-slate-50 hover:bg-white border-2 border-slate-300 hover:border-blue-500 rounded-xl p-3 shadow-xs transition-all text-slate-800 cursor-pointer group"
          >
            <Search className="h-5 w-5 text-blue-600 shrink-0 ml-1 group-hover:scale-110 transition-transform" />
            <div className="flex-1 px-3 text-xs sm:text-sm font-medium text-slate-700">
              {searchTerm ? (
                ...
                <span className="text-slate-400">Search {boardActiveJobs.length} active {boardName} jobs by title, post, qualification...</span>
              )}
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1 bg-blue-500/10 text-blue-700 border border-blue-500/30 rounded-lg text-xs font-black">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-black text-slate-800 flex items-center gap-2">
            <span>{boardName} Active Jobs</span>
        ...
        {filteredJobs.length > 0 ? (
          <div className="space-y-4">
            {viewMode === 'card' ? (
              <div className="space-y-3">
                {filteredJobs.slice(0, visibleCount).map((job, idx) => (
                  <JobCard key={`board-card-${job.id || idx}`} job={job} />
                ))}
              </div>
            ) : (
            ...
            {filteredJobs.length > visibleCount && (
              <div className="pt-4 flex flex-col items-center justify-center gap-2">
                ...
                >
                  <span>Show More {boardName} Jobs ({filteredJobs.length - visibleCount} remaining)</span>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center max-w-xl mx-auto space-y-4">
            ...
            <p className="text-xs text-slate-500 font-medium leading-relaxed">
              We couldn't find any active {boardName} notifications matching "<span className="font-bold text-slate-700">{searchTerm}</span>".
            </p>
            <button 
